...
# 0. 网页配置与参数设置
# ==========================================
st.set_page_config(page_title="FeliEase 药物筛选分析平台", layout="wide")

st.title("💊 FeliEase 高通量药物筛选分析平台")
st.markdown("""
此工具用于自动化分析药物筛选数据。
- **支持多板分析**：自动识别板号（每板间隔默认11行）。
- **智能清洗**：自动剔除内参异常值和毒性假阳性。
- **自动归一化**：基于每板内参计算 Ratio。
""")
...
if uploaded_file is not None:
    try:
        ...
        if all_results:
            ...
            col3.metric("高信号剔除", total_high_drop)
            ...
            if not hits_df.empty:
                ax.scatter(hits_df["Global_ID"], hits_df["Ratio"], c='red', s=40, label='Hits')
            ...
            ax.set_ylim(bottom=-0.1, top=df_res["Ratio"].max()*1.1)
            ax.legend()
            ...
            # 3. Hits 下载
            st.divider()
            st.subheader("🎉 强效药 (Hits) 列表")
            
            if not hits_df.empty:
                st.write(f"发现 **{len(hits_df)}** 个 Hits (Ratio < {THRESHOLD_HIT})")
                
                # 展示前10个
                hits_display = hits_df.sort_values(by="Ratio")[["Plate_ID", "Coordinate", "Ratio", "Raw_RFU", "Global_ID"]]
