...
# 0. 网页配置与参数设置
# ==========================================
st.set_page_config(page_title="FeliEase 药物筛选分析平台 (抑制剂模式)", layout="wide")

st.title("🧪 FeliEase 高通量药物筛选分析平台 - 抑制剂/信号下降筛选")
st.markdown("""
此工具用于自动化分析药物筛选数据，**寻找引起信号显著下降的化合物**。
- **自动定位板位**：彻底无视空行，自动巡航识别每板位置。
- **智能清洗**：自动剔除内参异常值和毒性假阳性。
- **极速映射**：请上传使用本地工具清洗后的 **Cleaned_For_Web 标准信息表**，一秒匹配药物名称！
""")
...
with st.sidebar:
    ...
    st.divider()
    # 【抑制剂专用】滑块范围调整：默认寻找 < 0.4 的药物
    THRESHOLD_HIT = st.slider("Hit 判定阈值 (Ratio < 该值)", 0.0, 1.0, 0.4, 0.05)
    LIMIT_HIGH_SIGNAL = st.number_input("高信号异常剔除倍数 (防沉淀/杂质)", value=3.5)
...
with col_upload2:
    uploaded_lib = st.file_uploader("2️⃣ 可选：上传本地清洗好的标准化合物库表", type=["xlsx", "csv"])
...
if uploaded_file is not None:
    try:
                ...
                if i + int(PLATE_HEIGHT) <= len(df):
                    ...
                    if plate_ctrl_mean > 0:
                        ...
                        for (rel_r, abs_c), cell_val in drug_block.stack().items():
                            ...
                            physical_well = f"{plate_row_char}{abs_c:02d}" # 生成 A02, B05...
        ...
        if all_results:
            ...
            col3.metric("高信号异常剔除", total_high_drop)
            ...
            ax.set_ylim(bottom=-0.1, top=max(y_max * 1.1, 1.2))
            ax.legend()
            ax.grid(True, linestyle=':', alpha=0.4)
            st.pyplot(fig)
            
            st.divider()
            st.subheader("🎉 强效抑制剂 (Hits) 列表")
            ...
            if not hits_df.empty:
                ...
                # 抑制剂模式：按 Ratio 升序排列 (越低的排在越前面)
                hits_display = hits_df.sort_values(by="Ratio", ascending=True)
                ...
                st.download_button(
                    ...
                    file_name="Inhibitor_Hits_with_Info.xlsx",
